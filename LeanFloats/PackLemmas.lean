...
theorem unpack_beq_unpack_iff {spec : Format} {b b' : BitVec spec.numBits}
  ...
  simp [unpack_eq_notANumber_iff_of_valid, unpack_eq_zero_iff_of_valid, unpack_inj_of_valid, hb, hb', hspec]

end Float.Model.UnpackedFloat

def Float32.nan : Float32 := 0/0
...
theorem Float.ofBits_toBits (f : Float) : ofBits f.toBits = f :=
  toBits_inj.mp <| UInt64.toBitVec_inj.mp <| Float.Model.UnpackedFloat.pack_unpack_of_valid f.toModel.valid

theorem Float32.ofBits_toBits (f : Float32) : ofBits f.toBits = f :=
  toBits_inj.mp <| UInt32.toBitVec_inj.mp <| Float.Model.UnpackedFloat.pack_unpack_of_valid f.toModel.valid

@[simp]
theorem Float.isNaN_iff_eq_nan {f : Float} : isNaN f ↔ f = nan := by
  rw [← toBits_inj, ← UInt64.toBitVec_inj]
  exact Float.Model.UnpackedFloat.isNaN_unpack_iff_of_valid f.toModel.valid

@[simp]
theorem Float32.isNaN_iff_eq_nan {f : Float32} : isNaN f ↔ f = nan := by
  rw [← toBits_inj, ← UInt32.toBitVec_inj]
  exact Float.Model.UnpackedFloat.isNaN_unpack_iff_of_valid f.toModel.valid

theorem Float.beq_iff_toBits_eq {f f' : Float} :
...
theorem Float32.beq_iff_toBits_eq {f f' : Float32} :
    f == f' ↔ f ≠ nan ∧ f' ≠ nan ∧ (f = f' ∨ (f = 0 ∧ f' = -0) ∨ (f = -0 ∧ f' = 0)) := by
...
theorem Float.nan_beq_eq_false (f : Float) : (nan == f) = false := by
  simp [← Bool.not_eq_true, beq_iff_toBits_eq]
...
instance : PartialEquivBEq Float where
  symm := by grind [Float.beq_iff_toBits_eq]
  trans := by grind [Float.beq_iff_toBits_eq]

instance : PartialEquivBEq Float32 where
  symm := by grind [Float32.beq_iff_toBits_eq]
  trans := by grind [Float32.beq_iff_toBits_eq]

example : Float.nan != Float.nan := by
  -- rfl
  simp
